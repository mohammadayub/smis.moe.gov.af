...
{
    public class SearchPrintedPassportsQuery : IRequest<List<SearchedProfileModel>>
    {
    ...
    public class SearchPrintedPassportsQueryHandler : IRequestHandler<SearchPrintedPassportsQuery, List<SearchedProfileModel>>
    {
        private AppDbContext Context { get; }
        private ICurrentUser CurrentUser { get; }
        public SearchPrintedPassportsQueryHandler(AppDbContext dbContext,ICurrentUser currentUser)
        {
            Context = dbContext;
            CurrentUser = currentUser;
        }
        public async Task<List<SearchedProfileModel>> Handle(SearchPrintedPassportsQuery request, CancellationToken cancellationToken)
        {
            var UserId = await CurrentUser.GetUserId();

            var query = Context.PassportPrints.Where(e => e.StatusId == PassportPrintStatus.Printed)
                .Where(e => e.PrintQueue.Application.CurProcessId == SystemProcess.Close)
                .AsQueryable();
            ...
            else
            {
                ...
                {
                    query = query.Where(e => e.PrintQueue.Application.StatusId == ApplicationStatus.Active);
                }
