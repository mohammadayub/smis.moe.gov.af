...
namespace Clean.Common.Dates
{
    public class PersianDate
    {
        ...
        public static string GetFormatedString(DateTime? Date)
        {
            ...
            if (Date != null)
            {
                PersianDate PD = PersianDate.Convert(Date.Value);
        ...
        // to change persian date to gregorian date
        public static DateTime ToGregorianDate(string persianDate)
        {
            persianDate = persianDate
                ...
                .Replace("۸", "8")
                .Replace("۹", "9");
            var year = System.Convert.ToInt32(persianDate.Substring(0, 4));
            var month = System.Convert.ToInt32(persianDate.Substring(5, 2));
            var day = System.Convert.ToInt32(persianDate.Substring(8, 2));
            return PersianDate.ToDate(year, month, day, 0, 0, 0, 0);
        }

        public static string ToPersianDate(DateTime gregorianDate)
        {
            var persianDate = PersianDate.Convert(gregorianDate).DateString;
            persianDate = persianDate
                .Replace("0", "۰")
                .Replace("1", "۱")
                .Replace("2", "۲")
