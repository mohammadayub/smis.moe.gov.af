...
{
    public class IndexModel : UploadBasePage
    {
        public async Task OnGetAsync()
        {
            ...
            ListOfBlackListReasons = new List<SelectListItem>();
            var reasons = await Mediator.Send(new GetBlackListReasonsQuery());
            reasons.ForEach(e => ListOfBlackListReasons.Add(new SelectListItem { Value = e.ID.ToString(), Text = String.Concat(e.Name, " (", e.NameEn, ")") }));

            ListOfOrganization = new List<SelectListItem>();
            var orgs = await Mediator.Send(new GetOrganiztionQuery());
            orgs.ForEach(e => ListOfOrganization.Add(new SelectListItem { Value = e.Id.ToString(), Text = e.Dari }));
        }
        ...
        public async Task<IActionResult> OnPostSave([FromBody] SaveBlackListProfileCommand command)
        {
            try
            {
        ...
        public async Task<IActionResult> OnPostSearch([FromBody] SearchBlackListProfileQuery query)
        {
            try
            {
