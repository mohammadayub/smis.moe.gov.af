...
<td class="border-0" style="{{monthTdStyle}}" {{monthTdAttribute}} data-td-month>
    ...
    function getSelectedDateTimeTextFormatted(setting) {
        ...
        if (setting.rangeSelector && setting.rangeSelectorStartDate != undefined && setting.rangeSelectorEndDate != undefined)
            return getDateTimeString(!setting.isGregorian ? getDateTimeJsonPersian1(setting.rangeSelectorStartDate) : getDateTimeJson1(setting.rangeSelectorStartDate), setting.textFormat, setting.isGregorian, setting.englishNumber, setting.toPersian) + ' - ' +
                getDateTimeString(!setting.isGregorian ? getDateTimeJsonPersian1(setting.rangeSelectorEndDate) : getDateTimeJson1(setting.rangeSelectorEndDate), setting.textFormat, setting.isGregorian, setting.englishNumber, setting.toPersian);
        return getDateTimeString(!setting.isGregorian || setting.toPersian ? getDateTimeJsonPersian1(setting.selectedDate) : getDateTimeJson1(setting.selectedDate), setting.textFormat, setting.isGregorian, setting.englishNumber, setting.toPersian);
    }
    ...
    function getDateTimeString(dateTimeJson, format, isGregorian, englishNumber, toPersian) {

        if (isGregorian) englishNumber = true;
        if (toPersian) englishNumber = false;
...
<div class="col-3 text-center" ${selectedDateTimeToShowJson.year == i ? 'selected-year' : ''}>
    ...
    $('html').on('click', function (e) {
        if (triggerStart)
            return;
        var $target = $(e.target), $popoverDescriber = getPopoverDescriber($target);

        if ($popoverDescriber.length >= 1)
            return;
        hidePopover($(mdDatePickerPopoverSelector));
    });
